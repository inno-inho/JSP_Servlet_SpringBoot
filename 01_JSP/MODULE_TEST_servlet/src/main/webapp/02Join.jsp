...
<!-- 자바파일에서 선언한 MemberDto와 DbUtils를 쓰기위한 불러오기, 그리고 정규식도 -->
<%@ page import="Dtos.*, Utils.*, java.util.regex.*" %>		

<%
	/* 문제 : 문자셋 설정 */
	
	request.setCharacterEncoding("UTF-8");		// 요청할 때 
	response.setContentType("text/html; charset=UTF-8");	// 응답받을 때
%>
...
<%!
	...
	public boolean isValid(MemberDto dto){
		...
		if(dto.getUserid().isEmpty()){		// 입력한 곳에 공란이 있다면
		...
		if(dto.getUserid().length()<=5){			// 입력한 ID가 5글자 이하라면
			System.out.println("Userid는 5자보다 많이 입력하셔야 합니다.");
			return false;
		}
...
<!-- 자바빈즈는 setter랑 getter 자동으로 생성해줌 -->
